...
def create_account():
    ...
    try:
        ...
        if connection.is_connected():
            ...
            cursor.execute("SELECT * FROM Create_account where gmail_user=%s", (email,))
...
def confirm_email(token):
    try:
        data = s.loads(token, salt=app.config['SECURITY_PASSWORD_SALT'], max_age=86400)  # max_age = 1 day
        email = data['email']
        ...
        try:
            ...
            if connection.is_connected():
                ...
                user = cursor.fetchall()
                if not user:
                    cursor.execute("INSERT INTO Create_account (name_account, name_user, password, gmail_user) VALUES (%s,%s,%s,%s)",
                                   (account_name, name_user, password_user, email))
                    connection.commit()
                    cursor.close()
...
#             user = cursor.fetchall()
#             if not user:
#                 token = s.dumps({'email': email}, salt=app.config["SECURITY_PASSWORD_SALT"])
#                 msg = Message('Xác nhận tài khoản', sender=app.config['MAIL_USERNAME'], recipients=[email])
#                 link = url_for('confirm_email', token=token, _external=True) # link đến route 'confirm'
#                 msg.body = "Your confirmation link is " + link
#                 mail.send(msg)
#                 message = {'status': 200, 'message': "Please check your email or spam"}
#                 connection.commit()
...
#         if connection.is_connected():
#             curs = connection.cursor()
#             curs.execute("INSERT INTO Create_account (name_account, name_user, password, gmail_user) VALUES (%s,%s,%s,%s)", (account_name, name_user, password_user, email))
#             connection.commit()
#             curs.close()
#             connection.close()
#             data = 'Favorite added successfully!'
