...
def get_All_anh_cua_tung_trang(linkpage): #lay link
    ...
    return mangaImg
@app.route("/index/<indexpage>", methods=["GET"]) #indexpage la thu tu cua trang. VD: http://192.168.1.11:5000/index/1 
...
@app.route('/New-Update')
def get_new_update():
    link = 'https://www.ninemanga.com/list/New-Update/'
    request_new_Update = requests.get(link)
    soup = BeautifulSoup(request_new_Update.text,'html.parser')
    list_new_update=[]
    for direlist in soup.find_all('ul', class_='direlist'):
        for dl in direlist.find_all('dl',class_='bookinfo'):
            new_update = dict()
            new_update['Name_Manga_Update'] = dl.find('a',class_='bookname').text
            dt = dl.find('dt')
            new_update['Image'] = dt.find('img').get('src')
            new_update['Chapter_name'] = dl.find('a',class_='chaptername').text
            new_update['Views'] = dl.find('span').text
            new_update['Descript'] = dl.find('p').text
            list_new_update.append(new_update)
    return jsonify(list_new_update)
@app.route('/Hot-Book')
def get_hot_manga():
    link = 'https://www.ninemanga.com/list/Hot-Book/'
    request_hot_manga = requests.get(link)
    soup = BeautifulSoup(request_hot_manga.text,'html.parser')
    list_hotmanga=[]
    for direlist in soup.find_all('ul', class_='direlist'):
        for dl in direlist.find_all('dl',class_='bookinfo'):
            hot_Manga = dict()
            hot_Manga['Name_Manga_Hot'] = dl.find('a',class_='bookname').text
            dt = dl.find('dt')
            hot_Manga['Image'] = dt.find('img').get('src')
            hot_Manga['Chapter_name'] = dl.find('a',class_='chaptername').text
            hot_Manga['Views'] = dl.find('span').text
            hot_Manga['Descript'] = dl.find('p').text
            list_hotmanga.append(hot_Manga)
    return jsonify(list_hotmanga)
@app.route('/New-Book')
def get_new_book():
    link = 'https://www.ninemanga.com/list/New-Book/'
    request_new_Book = requests.get(link)
    soup = BeautifulSoup(request_new_Book.text,'html.parser')
    list_new_manga=[]
    for direlist in soup.find_all('ul', class_='direlist'):
        for dl in direlist.find_all('dl',class_='bookinfo'):
            new_manga = dict()
            new_manga['Name_Manga_New'] = dl.find('a',class_='bookname').text
            dt = dl.find('dt')
            new_manga['Image'] = dt.find('img').get('src')
            new_manga['Chapter_name'] = dl.find('a',class_='chaptername').text
            new_manga['Views'] = dl.find('span').text
            new_manga['Descript'] = dl.find('p').text
            list_new_manga.append(new_manga)
    return jsonify(list_new_manga)
        ...
        if li is not None:
            for li2 in ul. find_all('li'):
                for dd in li2.find_all('dl', class_='bookinfo'):
                    ...
                    manga_search['Views'] = dd.find('span').texts
...
if __name__ == "__main__":
   app.run(host='0.0.0.0',port=19899999)
