...
def createcomment():
    noi_dung = request.headers.get('noi_dung_cmt')
    device_cmt = request.headers.get('device_cmt')
    id_toan_bo_su_kien = request.headers.get("id_toan_bo_su_kien")
    ipComment = request.headers.get("ipComment")
    thong_tin={}
    list_thong_tin=[]
    config = {
                'user': 'leooRealman',
    ...
    try:
        ...
        sql = f"INSERT INTO comment VALUES ( {result_id_sk[0][0]+1}, %s, %s , %s  )"
        val = (noi_dung , ipComment , device_cmt)
        thong_tin["id_comment"]=result_id_sk[0][0]+1
        thong_tin["noi dung cmt"]= noi_dung
        thong_tin["device cmt"]=device_cmt
...
def getPageLoveHistory(trang):
    ...
    try:
        ...
        mycursor.execute(f"SELECT * FROM saved_sukien  LIMIT 25 offset {(trang-1) *25}")
        result2 = mycursor.fetchall()
        print("kq2" ,result2)
        for i in range(0, 25):
            thong_tin["id"] = result2[i][0]
...
def getPageCommentHistory(trang):
    ...
    try:
        ...
        mycursor.execute(f"SELECT * FROM comment  LIMIT 50 offset {(trang-1) *50}")
        result2 = mycursor.fetchall()
        print("kq2" ,result2)
        for i in range(0, 4):
            thong_tin["id"] = result2[i][4]

            thong_tin["noi dung cmt"] = result2[i][1]


            thong_tin["dia chi ip"] = result2[i][2]


            thong_tin["device cmt"] = result2[i][3]
