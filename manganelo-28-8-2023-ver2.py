...
def change_password():
    ...
    try:
        ...
        if user_info:
            ...
            # Assume the hashed password is stored in the third column of the table.
            if bcrypt.checkpw(
                ...
                return {"ketqua": "Mật khẩu cũ không đúng."}
        else:
            return {"ketqua": "Email không tồn tại trong hệ thống."}
...
def get_DetailManga():
    ...
    soup1 = BeautifulSoup(request_ses.content, "html.parser")

    detail = {}
    # data1 = soup.find("table", class_="variations-tableInfo")
    lis = []

    list_chapter = soup1.find_all("div", class_="content-homepage-item")
    for chapter in list_chapter:
        link = "https://ww5.manganelo.tv" + chapter.find("a")["href"]
        request_se = requests.get(link)
        soup = BeautifulSoup(request_se.content, "html.parser")
        data1 = soup.find("table", class_="variations-tableInfo")
        ...
        # LẤY POSTER
        poster_element = soup.find("div", class_="story-info-left").find(
            "img", class_="img-loading"
        )
        if poster_element is not None:
            detail["poster_manga"] = "https://ww5.manganelo.tv" + poster_element["src"]
        else:
            detail["poster_manga"] = "N/A"
        df = soup.find_all("h1")
        # LẤY TIÊU ĐỀ
        for i in df:
...
#             link1 = "https://ww5.manganelo.tv" + link.find("a")["href"]
...
#             #     return link_imgs
#             request_s = requests.get(link1)
#             soup2 = BeautifulSoup(request_s.content, "html.parser")
#             data_img = soup2.find("div", class_="container-chapter-reader").find_all(
#                 "img", class_="img-loading"
#             )
#             for i in data_img:
#                 # link_imgs.append(item.get("data-src"))
...
#             #     soup = BeautifulSoup(request.text, "html.parser")
#             #     name_imgs = (
#             #         soup.find("div", class_="panel-chapter-info-top").find("h1").text
#             #     )
...
def add_favorite(iduser):
    ...
    link = data.get("link")
    dateTimeLike = data.get("dateTimeLike")
    ip_like = data.get("ip_like")
    name_device_like = data.get("name_device")
...
def add_recent(iduser):
    data = request.get_json()
    link_chapter = data.get("link_chapter")
    link_manga = data.get("link_manga")
    datetime = data.get("datetime")
    ip_readed = data.get("ip_readed")
    name_device_readed = data.get("name_device")
    try:
        connection = mysql.connector.connect(**config)
        if connection.is_connected():
            curs = connection.cursor()
...
def add_comment(iduser):
    ...
    NoiDungComment = data.get("NoiDungComment")
    link_image_attach = data.get("link_image_attach")
    dateTimeComment = data.get("dateTimeComment")
    nameDevice_Comment = data.get("name_device")
    IPComment = data.get("IPComment")
    try:
        connection = mysql.connector.connect(**config)
        if connection.is_connected():
            curs = connection.cursor()
