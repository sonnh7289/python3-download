...
@FLASKapp.route("/post_comments", methods=["POST"])
def post_comments():
    connection= mysql.connector.connect(**config)
    cursor = connection.cursor()
    noidung_comment = request.form.get('noidung_comment')
    ...
    insert_comment_query = "INSERT INTO comment_image (noidung_comment, timestamp , email_account , ip_comment , device_comment , image_link , id_image) VALUES (%s, %s,%s,%s,%s,%s,%s)"
    cursor.execute(insert_comment_query, (noidung_comment, timestamp, email_account , ip_comment, device_comment ,image_link ,id_image))
    connection.commit()
    cursor.close()
    connection.close()
    
@FLASKapp.route("/post_image", methods=["POST"])
...
if __name__ == '__main__':
    FLASKapp.run(host='0.0.0.0', port=8989)
