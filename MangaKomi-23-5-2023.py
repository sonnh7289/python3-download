...
def get_home():
    ...
    linksend = "https://mangakomi.io/" + link_full 
    request_ses = session.get(linksend)
    ...
    for manga in soup.findAll('div', class_='col-12 col-md-6 badge-pos-1'):
        ...
        linkMangaJSON = link_check.replace("https://mangakomi.io/", "") 
        manga_info['link_manga'] = linkMangaJSON
        #Lấy tên manga
        manga_info['title_manga'] = manga.find('a').get('title')

        #Lấy ảnh manga
        poster_manga = manga.find('img').get('data-src')
        poster_remove_manga = poster_manga.replace("https://mangakomi.io/", "") 
        manga_info['poster_manga'] = poster_remove_manga
        ...
        if span1 != None:
            lastest_chapter_manga = span1.find('a').get('href')
            linkMangaJSON = lastest_chapter_manga.replace("https://mangakomi.io/", "") 
            manga_info['lastest_chapter_manga'] = linkMangaJSON
...
def get_DetailManga():
    ...
    #Lấy anh manga
    poster_manga = soup.find('div', class_='summary_image').find('img').get('data-src')
    poster_remove_manga = poster_manga.replace("https://mangakomi.io/", "") 
    detail['poster_manga'] = poster_remove_manga
    ...
    for chap in soup.findAll('li', class_='wp-manga-chapter'):
        chapter = {}
        chapterLink = chap.find('a').get('href')
        chapter_remove_Link = chapterLink.replace("https://mangakomi.io/", "") 
        chapter['link_chapter'] = chapter_remove_Link
...
def search():
    ...
    for item in soup.findAll('div', class_='row c-tabs-item__content'):
        info = {}
        #Lay link manga
        link_manga = item.find('a').get('href')
        chapter_remove_Link = link_manga.replace("https://mangakomi.io/", "") 
        info['link_manga'] = chapter_remove_Link
        #Lay ten manga 
        info['title_manga'] = item.find('a').get('title')
        #Lay poster
        poster_manga = item.find('img').get('data-src')
        chapter_remove_Link = poster_manga.replace("https://mangakomi.io/", "") 
        info['poster_manga'] = chapter_remove_Link
        ...
        if sonpro != None:
            find1 = sonpro.find('a')
            if find1 != None:
                lastest_chapter_manga = find1.get('href') 
                chapter_remove_Link = lastest_chapter_manga.replace("https://mangakomi.io/", "") 
                info['lastest_chapter_manga'] = chapter_remove_Link
...
def category():
    ...
    for item in soup.findAll('div', class_='col-12 col-md-6 badge-pos-1'):
        ...
        lastest_chapter_manga = item.find('span', class_='chapter font-meta').find('a').get('href')
        chapter_remove_Link = lastest_chapter_manga.replace("https://mangakomi.io/", "") 
        info['lastest_chapter_manga'] = chapter_remove_Link
        #Lay ngay phat hanh
    ...
    if soup.find('div', class_= 'wp-pagenavi') is not None:        
        ...
        for a in pagege:
            ...
            chapter_remove_Link = link_page.replace("https://mangakomi.io/", "") 
            pageItem['link_page'] = chapter_remove_Link
