...
def get_celebrity():
    name_actor = request.headers.get("link-celebrity")
    actor = name_actor.strip().replace(" ", "_")
    session = requests.Session()
    link_actor = "https://www.rottentomatoes.com/celebrity/" + actor
    try:
        actor_base = session.get(link_actor)
        # check link
        if actor_base.status_code != 200:
            actor = name_actor.strip().replace(" ", "-")
            link_actor = "https://www.rottentomatoes.com/celebrity/" + actor
            actor_base = session.get(link_actor)
    except:
    ...
    info_actor = soup_actor_base.find("div", class_="celebrity-bio__content")
    item_actor = dict()
    item_actor["name"] = info_actor.find("h1").text.strip()
    item_actor["poster_actor"] = "http" + \
                                 info_actor.find("a", class_="celebrity-bio__hero-mobile").attrs["style"].split(
                                     "url(\"")[1].split("\")")[0].split("http")[-1]
    try:
        item_actor["summary"] = info_actor.find("p", class_="celebrity-bio__summary").text.strip()
    except:
        item_actor["summary"] = "Not Available"
...
# @app.route("/franchise/<string:name_film>", methods=["GET"])
# def get_franchise(name_film):
@app.route("/franchise", methods=["GET"])
def get_franchise():
    name_film = request.headers.get("link-film")
    new_name_film = name_film.strip().replace(" ", "_")
    session = requests.Session()
    link_film = "https://www.rottentomatoes.com/franchise/" + new_name_film
    try:
        film_base = session.get(link_film)
        if film_base.status_code != 200:
            new_name_film = name_film.strip().replace(" ", "-")
            link_film = "https://www.rottentomatoes.com/franchise/" + new_name_film
            film_base = session.get(link_film)
    except:
