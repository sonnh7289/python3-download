...
def get_Home():
    ...
    listJsonManga.append(listPopularUpdatesManga)
    l = []
...
def get_DetailManga():
    # link_full = requests.headers.get('Link-Full')
    session = requests.Session()
    rManga_base = session.get('https://www.mngdoom.com/zero-game')
    soupManga_base = BeautifulSoup(rManga_base.content, 'html.parser')
...
def get_Latest():
    ...
    listPopularUpdatesManga = []
    session = requests.Session()
    rManga_base = session.get('https://www.mngdoom.com/latest-chapters')
    soupManga_base = BeautifulSoup(rManga_base.content, 'html.parser')
    l = []
    for itemMangaLastUpdate in soupManga_base.findAll('dl'):
        Popular = dict()
        ...
        #LẤY CÁC CHAPTER
        itemMangaLastUpdate = itemMangaLastUpdate.findAll('dd')

        if itemMangaLastUpdate is not None:
            lis = []
            for itemManga in itemMangaLastUpdate:
                lis.append(itemManga.text.strip())
                Popular['chapter']  = lis
            lis = []
        ...
        if itemMangaLastUpdate is not None:
            lis = []
            for itemManga in itemMangaLastUpdate:
                lis.append(itemManga.a['href'])
                Popular['link_chapter']  = lis
            lis = []
