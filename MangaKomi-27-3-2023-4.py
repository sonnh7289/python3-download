...
def get_home():
    ...
    for manga in soup.findAll('div', class_='col-12 col-md-6 badge-pos-1'):
        ...
        #Lấy link manga           
        son = encrypt( manga.find('a').get('href'))
        print (son)
        manga_info['link_manga'] = son

        #Lấy tên manga
        manga_info['title_manga'] = manga.find('a').get('title')

        #Lấy ảnh manga
        manga_info['poster_manga'] = encrypt( manga.find('img').get('data-src'))
        ...
        if span1 != None:
            manga_info['lastest_chapter_manga'] = encrypt(span1.find('a').get('href'))
...
def get_DetailManga():
    ...
    #Lấy anh manga
    detail['poster_manga'] = encrypt(soup.find('div', class_='summary_image').find('img').get('data-src'))
    ...
    for chap in soup.findAll('li', class_='wp-manga-chapter'):
        chapter = {}
        chapter['link_chapter'] = encrypt(chap.find('a').get('href'))
...
def get_chapter():
    ...
    if soup.find('div', class_='nav-previous') is not None:
        img['prev'] = encrypt(soup.find('div', class_='nav-previous').find('a').get('href'))
    for num in soup.findAll('div', class_='page-break no-gaps'):
        num_img.append(encrypt(num.find('img').get('data-src').strip()))
    img['image'] = num_img
    img['next'] = encrypt(soup.find('div', class_='nav-next').find('a').get('href'))
...
def search():
    ...
    for item in soup.findAll('div', class_='row c-tabs-item__content'):
        info = {}
        #Lay link manga
        info['link_manga'] = encrypt(item.find('a').get('href'))
        #Lay ten manga 
        info['title_manga'] = item.find('a').get('title')
        #Lay poster
        info['poster_manga'] = encrypt(item.find('img').get('data-src'))
        ...
        if sonpro != None:
            find1 = sonpro.find('a')
            if find1 != None:
                info['lastest_chapter_manga'] = encrypt(find1.get('href'))
...
if __name__ == "__main__":
   app.run(host='0.0.0.0', port=24883)
