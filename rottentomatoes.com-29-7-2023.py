...
def get_Detail():
    ...
    for div in soupflim_base.findAll("div", class_="cast-and-crew-item"):
        ...
        for ahref in div.findAll("a"):
            castItem["link"] = "https://www.rottentomatoes.com"+ahref.get("href")
        for img in div.findAll("img"):
...
def get_celebrity():
    ...
    if info_actor is not None:
        ...
        if item_actor_base is not None:
            ...
            list_info["infor_actor"] = item_actor
            try:
                list_highest_rated_movies = []
                soup_highest_rated_movies = soup_actor_base.find('section', class_="dynamic-poster-list")
                soup_movies = soup_highest_rated_movies.find('tiles-carousel-responsive-item')
                if soup_movies is not None:
                    soup_item_movies = soup_highest_rated_movies.findAll('tiles-carousel-responsive-item')
                    if soup_item_movies is not None:
                        for item in soup_item_movies:
                            list_movies = dict()
                            link_film = item.find("a")["href"]
                            if link_film is not None:
                                list_movies['film'] = 'https://www.rottentomatoes.com'+link_film.strip()
                            else:
                                list_movies['link_film'] = ""
                            poster = item.find('img')['src']
                            if poster is not None:
                                list_movies["poster"] = 'http'+poster.split('http')[-1].strip()
                            else:
                                list_movies["poster"] = ""
                            name_movie = item.find('span', class_="p--small")
                            if name_movie is not None:
                                list_movies["name_movie"] = name_movie.text.strip()
                            list_highest_rated_movies.append(list_movies)
                    list_info["list_highest_rated_movies"] = list_highest_rated_movies
                else:
                    list_highest_rated_movies = []
            except:
                list_highest_rated_movies = []
            try:
                list_photos = []
                soup_list_photos = soup_actor_base.find('section', id="photos-module")
                if soup_list_photos is not None:
                    soup_list_img = soup_list_photos.findAll("img")
                    for item in soup_list_img:
                        list_photos.append(item["src"].strip())
                else:
                    list_photos = []
                list_info['photos'] = list_photos
            except:
                list_photos = []
...
def get_franchise():
    ...
    if name_film is not None:
        Item_film['name'] = name_film.text.strip()
    else:
        Item_film['name'] = ""
    poster = soup_film_base.find("img", class_="franchise-hero__banner")["srcset"]
    if poster is not None:
        Item_film["poster"] = poster.split(" ")[-2]
    else:
        Item_film["poster"] = ""
    summary = soup_film_base.find("div", class_="franchise-hero__summary")
    if summary is not None:
        Item_film["summary"] = summary.text.strip().split("\n")[0]
    else:
        Item_film["summary"] = ""
    ...
    title = soup_film_base.find("a", class_="franchise-hero__rating-link")
    if title is not None:
        Item_film["title"] = title.text.strip()
    else:
        Item_film["title"] = ""
    link_title = soup_film_base.find("a", class_="franchise-hero__rating-link")["href"]
    if link_title is not None:
        Item_film["link_title"] = "https://www.rottentomatoes.com" + link_title.strip()
    else:
        Item_film["link_title"] = ""

    list_info_film_base["header"] = [Item_film]
    Item_film_features = dict()
    try:
        list_features_base = soup_film_base.find("section", id="franchise-features")
        if list_features_base is not None:
            header_features = list_features_base.find("h2")
            if header_features is not None:
                Item_film_features["header_features"] = header_features.text.strip()
            else:
                Item_film_features["header_features"] = ""
            poster_features = list_features_base.find("img")["srcset"]
            if poster_features is not None:
                Item_film_features["poster_features"] = poster_features.strip()
            else:
                Item_film_features["poster_features"] = ""
            title_features = list_features_base.find("p", class_="franchise-feature__title")
            if title_features is not None:
                Item_film_features["title_features"] = title_features.text.strip()
            else:
                Item_film_features["title_features"] = ""

            list_info_film_base["features"] = [Item_film_features]
        else:
            list_info_film_base["features"] = []
    except:
        list_info_film_base["features"] = []
    try:
        list_photos_base = soup_film_base.find("section", id="photos-module")
        if list_photos_base is not None:
            ...
            for item_tv_show in list_item_tv_show_base:
                ...
                if link_film is not None:
                    list_item_tv_show['link'] = "https://www.rottentomatoes.com" + link_film.strip()
                else:
                    list_item_tv_show['link'] = ""
