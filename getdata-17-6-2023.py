...
def createdata():
    ...
    filename1 = 'imgs/anhtam1' + timeString + '.jpg'
    filename2 = 'imgs/anhtam2' + timeString + '.jpg'
    filename3 = 'imgs/anhtam3' + timeString + '.jpg'
    filename4 = 'imgs/anhtam4' + timeString + '.jpg'
    output_ketqua1 = 'results/output1' + timeString + '.jpg'
    output_ketqua2 = 'results/output2' + timeString + '.jpg'
    outputFile = 'results/output' + timeString + '.jpg'
    ...
    if random_case == 0:
        while (True):
            ...
            try:
                ...
                random_sukien = ['skchiatay2', 'sknym', 'skmaymua', 'skkethon', 'skngoaitinh', 'sklyhon']
                ...
                print('result10', result10[0])
                my_string = ', '.join(result10[0])
                print('mystring', my_string)
                item1SuKien["tensukien"] = my_string
                ...
                if my_string1 == "0" and my_string2 == "0":
                    choose_case = 4
                    download_image(my_string12, outputFile)
                ...
                if choose_case == 1:
                    # Swap faces
                    args = argparse.Namespace(src= filename2 , dst=filename4, out= output_ketqua1,
                                              warp_2d=False,
                ...
                if choose_case == 2:
                    args = argparse.Namespace(src= filename1 , dst=filename3, out= output_ketqua1,
                                              warp_2d=False,
                                              correct_color=False, no_debug_window=True)
                    src_img = cv2.imread(args.src)
                    dst_img = cv2.imread(args.dst)
                    ...
                    if my_string13 == "namsau":
                        ...
                        for k, dst_face2 in dst_faceBoxes2.items():
                            output2 = face_swap(src_face2, dst_face2["face"], src_points2, dst_face2["points"],
                                                ...
                                                output2,
                                                args1)
                        output_path2 = output_ketqua2
                        cv2.imwrite(output_path2, output2)
                        for i in range(0, 11):
                            if (check_imgbb_api_key(list_API_KEY[i]) == True):
                                 api_key = list_API_KEY[i]
                                 break
                        image1 = Image.open(output_ketqua1)
                        image2 = Image.open(output_ketqua2)

                        image_1 = cv2.imread(output_ketqua1)
                        image_2 = cv2.imread(output_ketqua2)
        ...
        os.remove(filename3)
        os.remove(filename4)
        os.remove(output_ketqua1)
        os.remove(output_ketqua2)
    ...
    if random_case == 1:
        while (True):
            ...
            try:
                ...
                mycursor = connection.cursor()
                # mycursor.execute(sql, val)
                random_sukien = ['skchiatay2', 'sknym', 'skSinhConDauLong', 'skkethon', 'skToTinh', 'sklyhon']
                print(random_sukien[index_demo])
    ...
    if random_case == 2:
        while (True):
            ...
            try:
                ...
                mycursor = connection.cursor()
                # mycursor.execute(sql, val)
                random_sukien = ['sknym', 'skGapNhau', 'skngoaitinh', 'skmuasam', 'skkethon', 'sklyhon']
                print(random_sukien[index_demo])
                ...
                if choose_case == 1:
                    ...
                    cv2.imwrite(output_path, output)
                    # vitri=0
                    # if (check_imgbb_update(list_API_KEY, vitri=vitri) == True):
                    #     api_key = list_API_KEY[vitri]
                    for i in range (0 , 11):
...
def getPageLoveHistory(trang):
    ...
    try:
        ...
        soPhanTu = PhanTuMax[0][0]   
        if trang * 25 > soPhanTu:
            return {"messages" : "vuot qua so phan tu"}
        if trang < 1:
            return {"messages" : "page start from 1"}
        for idItemPhanTu in range(soPhanTu - ((trang - 1) * 25) - 25, soPhanTu - ((trang - 1) * 25)):
            Mot_LanQuerryData = []
...
@app.route("/lovehistory/add/<int:id_toan_bo_su_kien>")
def addThemSuKienTinhYeu(id_toan_bo_su_kien):
    ...
    try:
        ...
        date = datetime.now().strftime("%Y-%m-%d, %H:%M:%S")
        result1 = mycursor.fetchall()
        sql = f"INSERT INTO saved_sukien (id_saved , link_da_swap , thoigian_swap , ten_su_kien , noidung_su_kien , ip_them_su_kien, device_them_su_kien, id_toan_bo_su_kien ,so_thu_tu_su_kien) VALUES ( {id_saved_max}  ,%s  , %s  ,%s,%s, %s, %s, { id_toan_bo_su_kien },{so_thu_tu_su_kien})"
        val = (link_da_swap, date, ten_su_kien,noidung_su_kien,ip_them_su_kien , device_them_su_kien)
        mycursor.execute(sql, val)
    except mysql.connector.Error as error:
        print(f"Failed to connect to MySQL database: {error}")
        return {"ketqua":"Failed to connect to MySQL database:"}
    finally:
        if 'connection' in locals() and connection.is_connected():
            cursor.close()
            connection.close()
