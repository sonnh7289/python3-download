...
def get_chapter():
    ...
    if soup.find('div', class_='nav-previous') is not None:
        img['prev'] = base64.b64encode(bytes(soup.find('div', class_='nav-previous').find('a').get('href')), 'utf-8')
    for num in soup.findAll('div', class_='page-break no-gaps'):
        num_img.append(base64.b64encode(bytes( num.find('img').get('data-src').strip()), 'utf-8')    )
    img['image'] = num_img
    img['next'] = base64.b64encode(bytes(soup.find('div', class_='nav-next').find('a').get('href'), 'utf-8'))
...
def search():
    ...
    for item in soup.findAll('div', class_='row c-tabs-item__content'):
        info = {}
        #Lay link manga
        info['link_manga'] = base64.b64encode(bytes(item.find('a').get('href'), 'utf-8'))
        #Lay ten manga 
        info['title_manga'] = item.find('a').get('title')
        #Lay poster
        info['poster_manga'] = base64.b64encode(bytes(item.find('img').get('data-src'),'utf-8'))
        ...
        if sonpro != None:
            find1 = sonpro.find('a')
            if find1 != None:
                info['lastest_chapter_manga'] = base64.b64encode(bytes(find1.get('href') , 'utf-8'))
...
def category():
    ...
    for item in soup.findAll('div', class_='col-12 col-md-6 badge-pos-1'):
        ...
        info['lastest_chapter_manga'] = base64.b64encode(bytes(item.find('span', class_='chapter font-meta').find('a').get('href'), 'utf-8'))
        #Lay ngay phat hanh
    ...
    if soup.find('div', class_= 'wp-pagenavi') is not None:        
        ...
        for a in pagege:
            pageItem = dict()        
            pageItem['link_page']= base64.b64encode(bytes(a.get('href'),'utf-8') )
