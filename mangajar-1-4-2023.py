...
def get_Home():
    ...
    for homedemo in home_list.findAll('div', class_='card-body py-2'):
        ...
        for item_manga in article_class:
                ...
                span = div1_ItemJsonManga.findAll('span')
                
                for demoo in span:
                    if len(demoo) == 1:
                        ItemJsonManga['time_update'] = demoo.text
...
def get_topmanga():
    ...
    for article in soupManga_base.findAll('article', class_='flex-item card mx-1 mx-md-2 mb-3 shadow-sm rounded'):
        ...
        span = post_footer.findAll('span')                
        for demoo in span:
            if len(demoo) == 1:
                topmangaJsonManga['time_update'] = demoo.text
        listtopgenre.append(topmangaJsonManga)
    link_full_page['list_item'] = listtopgenre
...
def get_fullList():
    ...
    for list_item in soupManga_base.findAll('article', class_= 'flex-item card mx-1 mx-md-2 mb-3 shadow-sm rounded'):
        ...
        temp = 1
        for card_genres in list_item.findAll('li', class_= 'card-genres-item'):
            if temp == 1:
                item['genre_1'] = card_genres.text
            if temp == 2:
                item['genre_2'] = card_genres.text
            temp +=1
        ...
        text_span = li.findAll('span')
        for demoo in text_span:
                    if len(demoo) == 1:
                        item['time_update'] = demoo.text
...
def get_DetailManga():
    ...
    for chapter in soupManga_base.findAll('div', class_= 'container manga-container'):
        ...
        ratingResults = post_info.find('div', class_='ratingResults mr-3')
        FullDetailManga['ratingValue'] = ratingResults.find('b', class_ = 'ratingValue ml-1').text
        FullDetailManga['votes'] = ratingResults.find('small').text
        text_span = post_info.findAll('span')
        texttext = []
        number = 1
        text = dict()
        for demoo in text_span:
            ...
            if number == 1:
                text['ratesCount'] = demoo.text
            if number == 3:
                text['Subscribe'] = demoo.text
            if number == 5:
                text['Read_later'] = demoo.text
            if number == 6:
                text['chapter'] = demoo.text
            if number == 7:
                text['status'] = demoo.text
            
            if number == 9:                
                for link in demoo.findAll('a'):
                    list_link = dict()
                    list_link['linkliss'] = link.get('href')
                    list_link['name'] = link.text
                    linkkk.append(list_link)
                text['genres'] = linkkk
                    
            if number == 10:
                text['year'] = demoo.find('a').text
            number+=1
        texttext.append(text)
        FullDetailManga['content'] = texttext
         
         
        linkcontent =[]        
        mangamanga = chapter.find('div', class_ = 'manga-links btn-group pt-2')
        for mangalink in mangamanga.findAll('a'):
            linklink= dict()            
            linklink['link_chapter'] = mangalink.get('href')
            h2 = mangalink.find('h2')
            linklink['name_chapter'] = h2.text            
            linkcontent.append(linklink)  
            
         
        FullDetailManga['link'] = linkcontent
...
@app.route("/read_chapter", methods=["GET"])
def read_chapter():
    link_full = request.headers.get('Link-Full')
    print(link_full)
    session = requests.Session()
    ...
    image_read = []
    read_item = soupManga_base.find('div', class_= 'container-fluid chapter-container')
    title_read = read_item.findAll('div', class_='text-center')
    for read in title_read:
        reading = dict()
        reading['href']= read.find('a').get('href')
        reading['title']= read.find('a').text
        image_read.append(reading)
    read_chapter['title'] = image_read
    
    all_image = read_item.find('div', class_ = 'mt-1 d-flex flex-column align-items-center chapter-images')
    images = all_image.findAll('img')
    ...
    for image in images:
        image_chap = dict()
        if numb ==1:
            image_chap['data_src'] = image.get('src')
            image_chap['title_image'] = image.get('title')
        else:
            image_chap['data_src'] = image.get('data-src')
            image_chap['title_image'] = image.get('title')
        numb += 1
        image_chaps.append(image_chap)
    read_chapter['img'] = image_chaps
    
    
    information = []
    info = dict()
    last_chapter = read_item.find('div', class_= 'row text-center')
    info['last_chapter'] = last_chapter.find('h2').text
    info['current_chapter'] = last_chapter.find('span', class_ = 'current-chapter').text
    
    mang_a = []
    aa = last_chapter.findAll('a')
    for aaa in aa:
        aas = dict()
        aas['href_current']= aaa.get('href')
        aas['name']= aaa.text
        mang_a.append(aas)
    info['conten_a'] =  mang_a   
    information.append(info)
    
    read_chapter['text_center'] = information
