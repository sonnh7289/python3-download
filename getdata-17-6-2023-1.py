...
def addThemSuKienTinhYeu(id_toan_bo_su_kien):
    ...
    try:
        ...
        mycursor.execute(sql, val)
        ketqua = "(id_saved , link_da_swap , thoigian_swap , ten_su_kien , noidung_su_kien , ip_them_su_kien, device_them_su_kien,thoigian_sukien, id_toan_bo_su_kien ,so_thu_tu_su_kien) VALUES ( {id_saved_max}  , {link_da_swap}  , {thoigian_swap}  , {ten_su_kien}, {noidung_su_kien} , {ip_them_su_kien},  {device_them_su_kien} , {thoigian_sukien}, { id_toan_bo_su_kien },{so_thu_tu_su_kien})"
        thong_tin = {"ketqua":sql }
    except mysql.connector.Error as error:
        print(f"Failed to connect to MySQL database: {error}")
        return {"ketqua":"Failed to connect to MySQL database: {error}"}
    finally:
        if 'connection' in locals() and connection.is_connected():
            cursor.close()
            connection.close()
