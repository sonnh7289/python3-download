...
def get_home():
    ...
    for manga in soup.findAll('div', class_='col-12 col-md-6 badge-pos-1'):
        ...
        # print (linkEncrpyt)
        manga_info['link_manga'] = base64.b64encode(manga.find('a').get('href'))
    
        #Lấy tên manga
        manga_info['title_manga'] = manga.find('a').get('title')

        #Lấy ảnh manga
        manga_info['poster_manga'] = base64.b64encode(manga.find('img').get('data-src'))
        ...
        if span1 != None:
            manga_info['lastest_chapter_manga'] = base64.b64encode(span1.find('a').get('href'))
...
def get_DetailManga():
    ...
    #Lấy anh manga
    detail['poster_manga'] = base64.b64encode(soup.find('div', class_='summary_image').find('img').get('data-src'))
    ...
    for chap in soup.findAll('li', class_='wp-manga-chapter'):
        chapter = {}
        chapter['link_chapter'] = base64.b64encode(chap.find('a').get('href'))
...
def get_chapter():
    ...
    if soup.find('div', class_='nav-previous') is not None:
        img['prev'] = base64.b64encode(soup.find('div', class_='nav-previous').find('a').get('href'))
    for num in soup.findAll('div', class_='page-break no-gaps'):
        num_img.append(base64.b64encode(num.find('img').get('data-src').strip()))    
    img['image'] = num_img
    img['next'] = base64.b64encode(soup.find('div', class_='nav-next').find('a').get('href'))
...
def search():
    ...
    for item in soup.findAll('div', class_='row c-tabs-item__content'):
        info = {}
        #Lay link manga
        info['link_manga'] = base64.b64encode(item.find('a').get('href'))
        #Lay ten manga 
        info['title_manga'] = item.find('a').get('title')
        #Lay poster
        info['poster_manga'] = base64.b64encode(item.find('img').get('data-src'))
        ...
        if sonpro != None:
            find1 = sonpro.find('a')
            if find1 != None:
                info['lastest_chapter_manga'] = base64.b64encode(find1.get('href'))
...
def category():
    ...
    for item in soup.findAll('div', class_='col-12 col-md-6 badge-pos-1'):
        ...
        info['lastest_chapter_manga'] = base64.b64encode(item.find('span', class_='chapter font-meta').find('a').get('href'))
        #Lay ngay phat hanh
    ...
    if soup.find('div', class_= 'wp-pagenavi') is not None:        
        ...
        for a in pagege:
            pageItem = dict()        
            pageItem['link_page']= base64.b64encode(a.get('href'))
